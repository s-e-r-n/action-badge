...
const settleTimeMs = 140;
const settledGap = 0.002;

const clampUnit = (value: number) => Math.max(-1, Math.min(1, value));

export const createPose = (scene: HTMLElement): Pose => {
  ...
  const paint = () => {
    scene.style.setProperty("--gift-card-aim-x", current.x.toFixed(4));
    scene.style.setProperty("--gift-card-aim-y", current.y.toFixed(4));
  };
