...
const aimToward = (scene: HTMLElement, clientX: number, clientY: number): Aim => {
  const box = scene.getBoundingClientRect();
  const radius = Math.max(box.width, box.height) / 2;
  const offsetX = (clientX - (box.left + box.width / 2)) / radius;
...
export const followPage = ({ scene, pose }: { scene: HTMLElement; pose: Pose }) => {
  const surface = document.documentElement;
  const onMove = (event: PointerEvent) => pose.aim(aimToward(scene, event.clientX, event.clientY));
