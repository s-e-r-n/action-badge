...
| 2026-09-25 | The validated gift card wrote its copy, prices and mark label into the markup, and a folder in this repository carries no client content | `GiftCard` takes `lead`, `title`, `titleTail { text, emoji? }`, `price`, `formerPrice?`, `mark`, `markLabel`, `giftLabel`; its look stays fixed as validated, with no color parameter | |
| 2026-09-25 | The mark's proportions were written in the style module as `800 / 62.55`, those of one client's mark | `mark` is a `StaticImageData` and its `width / height` sets the ratio; Next.js rounds an SVG height to a whole pixel, under one percent of stretch for the published mark | |
| 2026-09-25 | The gift card's pose and drivers repeat the badge's math | Each module keeps its own copy and neither folder imports the other, the human's decision: they are two separate embeddable components | |
